...
$$
=======
$$\ln(R_i/S_i) = \log(\alpha) - \beta S_i $$


It's also common to parameterize this model using $S_{max} = 1/\beta$, which is the number of spawners that produces the maximum number of recruits (the peak of the curve,) since it is more intuitive to put priors around. 
$$\ln(R_i/S_i) = \log(\alpha) - S_i/S_{max}$$
>>>>>>> Stashed changes
...
SimData <- Sim_Ricker_SR_Data(leng=50, age=4, Sig_Ricker = 0.15, true_a = rnorm(1, 5, 2), true_b=1/100000, hr_min = 0.2, hr_max = 0.8, lnorm_corr = FALSE) # keep age=4
...
logRS <- log10(SimData$DF_Out$R / SimData$DF_Out$S)
fit.RS <- lm(logRS ~ S, data = SimData$DF_Out)
summary(fit.RS)
exp(coef(fit.RS)[1])
log(SimData$true_a)
...
If we think about the intercept, exponentiate it, then we compare it to the value of $\log(\alpha)$. According to the Ricker Model, they should be equal.
...
Once we know (or approximate) $S_{MSY}$ we can estimate $S_{gen}$ by numerically, solving the equation:
...
fsd2022 <- read.csv("DataIn/FraserSockeyeData2022.csv", header = TRUE)
fsd2022_sub <- fsd2022 %>% filter(`Pop_Name` == 'Early Stuart') %>%
  mutate(logRS = log10(R / S))
fit.2022 <- lm(logRS ~ S, data = fsd2022_sub)
summary(fit.2022)
```


```{r}
# a <- exp(as.numeric(coef(fit.2022)[1]))
out.2022 <- GetRickerBMs(SRDat = fsd2022_sub, a = NA, B = NA)

out.2022$SMSY
out.2022$Sgen

mean(fsd2022_sub$S) # average spawner numbers (average from last 4 years)
```
