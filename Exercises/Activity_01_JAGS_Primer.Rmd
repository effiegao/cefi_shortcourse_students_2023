...
Note: Anything unknown needs to have a prior.
...
-   Evaluate the MCMC chain for convergence. Include relevant diagnostics and plots. Determine and remove burn-in e.g. `var.burn <- window(var.out,start=burnin)`
...
x2 <- runif(n,0,15) # x2
x12 <- x1*x2
x <- cbind(rep(1,n),x1,x2,x1*x2) # interaction x1*x2
y <- rnorm(n,x%*%beta,sigma)
plot(x1,y)
plot(x2,y)
```
...
```{r}
data2 <- list(x1 = x1, x2 = x2, x12 = x12, y = y, n = n)
```
...
model{
  ...
  for(i in 1:n){
	  mu[i] <- beta[1] + beta[2]*x1[i] + beta[3]*x2[i] + beta[4]*x12[i]	        ## process model
	  y[i]  ~ dnorm(mu[i],prec)		        ## data model
  }
}
"
...
```{r}
## specify priors
data2$b0 <- as.vector(c(0,0,0,0))  ## regression beta means
data2$Vb <- solve(diag(10000,4))   ## regression beta precisions
...
## MCMC loop
```{r}
j2.model   <- jags.model(file = textConnection(multivariate_regression),
                             data = data2,
                             inits = inits,
                             n.chains = nchain)
```
...
```{r}
var2.out   <- coda.samples (model = j2.model,
                            variable.names = c("beta","prec"),
                                n.iter = 2000)
```
...
```{r}
## remember to assess convergence and remove burn-in before doing other diagnostics
GBR2 <- gelman.plot(var2.out)

## convert to matrix
var2.mat      <- as.matrix(var2.out)

## Pairwise scatter plots & correlation
pairs(var2.mat)	## pairs plot to evaluate parameter correlation
cor(var2.mat)    ## correlation matrix among model parameters
```
